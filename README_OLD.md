...
Sistema completo de gestão financeira para casais com integração WhatsApp, desenvolvido como Progressive Web App (PWA) instalável em dispositivos móveis.
...
O **Controle de Itens** funciona como uma lista de compras inteligente e sistema de controle de pagamentos, enquanto o módulo de **Faturamento** registra receitas diárias com visualização mensal consolidada. Os **Relatórios Avançados** incluem gráficos de despesas por pessoa e por categoria, com funcionalidade de exportação para CSV e navegação temporal por mês.

A **Integração WhatsApp** oferece webhook para recebimento de mensagens, parser de comandos em linguagem natural e notificações automáticas. O sistema é **Multi-usuário**, com suporte para múltiplos usuários (ideal para casais), identificação automática via WhatsApp e relatórios individualizados por pessoa.
...
Para instruções detalhadas, consulte o arquivo **[DEPLOY_RAILWAY.md](./DEPLOY_RAILWAY.md)**.
...
Após configurar a integração com Twilio, os usuários podem enviar comandos via WhatsApp:

- `gasto 50 mercado` - Adiciona despesa de R$ 50,00
- `gasto 150.50 restaurante` - Adiciona despesa de R$ 150,50
