  ...
  app.post("/api/trpc/whatsapp.webhook", async (req: Request, res: Response) => {
    try {
      ...
      if (cmd.type === "gasto") {
        ...
        // Notificar via WebSocket para sincronização em tempo real
        const { realtimeEvents } = await import("./realtime-events");
        realtimeEvents.expenseCreated(householdId, expense);
        realtimeEvents.whatsappCommandProcessed(householdId, "gasto", { success: true, expense });
